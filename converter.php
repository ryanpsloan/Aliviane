...
try {
    if ($_FILES['export']['error'] === 0) {
        ...
        $eeHoursWorked = $mHours = $filteredData = array();
        ...
        //var_dump($eeHoursWorked, $mHours, $filteredData, $mCodes);
        ...
        foreach($filteredData as $eeNum => $array){
            ...
            foreach($array as $key => $line){
                ...
                $calculationData[$eeNum][] = array("PERCENTAGE" => $percentage, "PROGRAM" => $program, "EE NAME" => $line["EE NAME"]);
            }
        }
        //var_dump($calculationData);
        ...
        foreach($mHours as $eeNum => $array){
            for ($i = 0; $i < count($mHours[$eeNum]); $i++) {
                ...
                $calculationData[$eeNum] = array_map(function($element) use ($hoursToDistribute, $earningCode){
                    $element["ACCRUAL"] = $element["PERCENTAGE"] * $hoursToDistribute;
                    $element["HOURS TO DISTRIBUTE"] = $hoursToDistribute;
                    $element["EARNING CODE"] = str_replace("M", "E", $earningCode);
                    //var_dump($element);
                    return $element;
                },$calculationData[$eeNum]);
            }
        }
        //var_dump($calculationData);

        foreach($calculationData as $eeNum => $array){
...
HTML;
            ...
            foreach($array as $key => $line){
                ...
                $program = $line["PROGRAM"];
                $percentage = round($line["PERCENTAGE"],2);
                $accrual = round($line["ACCRUAL"], 2);

                $ui[] = <<<HTML
                    <table class="table">
                        <thead>
                            <tr><th>Program</th><th>Percentage (%)</th><th>Distributed</th></tr>
                        </thead>
                        <tbody>
                            <tr><td>$program</td><td>$percentage</td><td>$accrual</td></tr>
                        </tbody>
                    </table>
HTML;
...
HTML;
        ...
        $_SESSION['output'] = array( "message" => "<p>iSolved Import File Successfully created!</p>", "link" => "<p><a href='./download.php'>Download Created File</a></p>", "ui" => $ui);
        $_SESSION['data'] = $data;
